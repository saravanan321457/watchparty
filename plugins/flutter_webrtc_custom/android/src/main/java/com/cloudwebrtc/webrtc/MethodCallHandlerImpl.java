...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  @Nullable ConstraintsMap androidAudioConfiguration, Severity logSeverity, @Nullable Integer audioSampleRate, @Nullable Integer audioOutputSampleRate, boolean enableWARP, boolean zeroPlayoutDelay) {
    ...
    // ── MP4 audio injection ──────────────────────────────────────────────
    // When the host is streaming a local MP4, Mp4AudioExtractor decodes the
    // audio track in parallel and feeds the raw PCM into WebRTC here.
    // The callback fires on the WebRTC audio thread; we simply fill the
    // ByteBuffer with decoded samples (or silence when no MP4 is active).
    audioDeviceModuleBuilder.setAudioBufferCallback((buffer, sampleRate, channelCount, framesPerBuffer, totalFrames, captureTimestampNs) -> {
